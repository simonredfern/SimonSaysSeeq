...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    grid_manager.set_led(main_grid_id, 15, 0, 15, "example_caller")?;
    grid_manager.refresh()?;
    thread::sleep(Duration::from_millis(3000));
    
    // Clear
    grid_manager.set_led(main_grid_id, 15, 0, 0, "example_caller")?;
    grid_manager.refresh()?;
    thread::sleep(Duration::from_millis(500));
    
    println!("Test 3: Single LED at grid coordinate (0,7) - should be BOTTOM-LEFT");
    grid_manager.set_led(main_grid_id, 0, 7, 15, "example_caller")?;
    grid_manager.refresh()?;
    thread::sleep(Duration::from_millis(3000));
    
    // Clear
    grid_manager.set_led(main_grid_id, 0, 7, 0, "example_caller")?;
    grid_manager.refresh()?;
    thread::sleep(Duration::from_millis(500));
    
    println!("Test 4: Single LED at grid coordinate (15,7) - should be BOTTOM-RIGHT");
    grid_manager.set_led(main_grid_id, 15, 7, 15, "example_caller")?;
    grid_manager.refresh()?;
    thread::sleep(Duration::from_millis(3000));
    
    // Clear
    grid_manager.set_led(main_grid_id, 15, 7, 0, "example_caller")?;
    grid_manager.refresh()?;
    thread::sleep(Duration::from_millis(500));
    
    println!("Test 5: Light up entire top row (row 0) - should be TOP ROW");
    for x in 0..16 {
        grid_manager.set_led(main_grid_id, x, 0, 12, "example_caller")?;
    }
    grid_manager.refresh()?;
    thread::sleep(Duration::from_millis(3000));
    ...
    // Clear
    for x in 0..16 {
        grid_manager.set_led(main_grid_id, x, 0, 0, "example_caller")?;
    }
    grid_manager.refresh()?;
    thread::sleep(Duration::from_millis(500));
    ...
    for x in 0..16 {
        grid_manager.set_led(main_grid_id, x, 7, 12, "example_caller")?;
    }
    grid_manager.refresh()?;
    thread::sleep(Duration::from_millis(3000));
    ...
    for y in 0..8 {
        grid_manager.set_led(main_grid_id, 0, y, 12, "example_caller")?;
    }
    grid_manager.refresh()?;
    thread::sleep(Duration::from_millis(3000));
    ...
    // Clear
    for y in 0..8 {
        grid_manager.set_led(main_grid_id, 0, y, 0, "example_caller")?;
    }
    grid_manager.refresh()?;
    thread::sleep(Duration::from_millis(500));
    ...
    for y in 0..8 {
        grid_manager.set_led(main_grid_id, 15, y, 12, "example_caller")?;
    }
    grid_manager.refresh()?;
    thread::sleep(Duration::from_millis(3000));
    ...
    // Clear
    for y in 0..8 {
        grid_manager.set_led(main_grid_id, 15, y, 0, "example_caller")?;
    }
    grid_manager.refresh()?;
    thread::sleep(Duration::from_millis(500));
