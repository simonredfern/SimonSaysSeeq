...
/// Test mode functionality for injecting button presses
pub struct TestModeInjector {
    button_a_file: String,
    button_b_file: String,
}

impl TestModeInjector {
    pub fn new() -> Self {
        Self {
            button_a_file: "button_a.txt".to_string(),
            button_b_file: "button_b.txt".to_string(),
        }
    }
    
    /// Check for and process test button injections
    /// Returns Some((grid_id, x, y, press/release)) if button should be injected
    pub fn check_injections(&self) -> Vec<TestButtonEvent> {
        let mut events = Vec::new();
        
        // Check button A file
        if let Ok(content) = std::fs::read_to_string(&self.button_a_file) {
            let content = content.trim();
            if content != "none" && !content.is_empty() {
                if let Some(event) = self.parse_button_command(content, "button_a") {
                    events.push(event);
                    // Clear the file after processing
                    let _ = std::fs::write(&self.button_a_file, "none");
                }
            }
        }
        
        // Check button B file
        if let Ok(content) = std::fs::read_to_string(&self.button_b_file) {
            let content = content.trim();
            if content != "none" && !content.is_empty() {
                if let Some(event) = self.parse_button_command(content, "button_b") {
                    events.push(event);
                    // Clear the file after processing
                    let _ = std::fs::write(&self.button_b_file, "none");
                }
            }
        }
    ...
    /// Parse button command from file content
    /// Format: "press,grid_id,x,y" or "release,grid_id,x,y" or "x,y" (assumes press on grid_one)
    fn parse_button_command(&self, content: &str, source_file: &str) -> Option<TestButtonEvent> {
        let parts: Vec<&str> = content.split(',').collect();
        
        match parts.len() {
            2 => {
                // Simple format: "x,y" - assume press on first grid
                if let (Ok(x), Ok(y)) = (parts[0].parse::<usize>(), parts[1].parse::<usize>()) {
                    log_test_injection("button_injection", &format!("{}:press,grid_one,{},{}", source_file, x, y));
                    return Some(TestButtonEvent {
                        grid_id: "grid_one".to_string(),
                        x,
                        y,
                        is_press: true,
                        source: if source_file == "button_a" { ButtonSource::TestInjectionA } else { ButtonSource::TestInjectionB },
                    });
                }
            },
            4 => {
                // Full format: "press/release,grid_id,x,y"
                let is_press = parts[0] == "press";
                let grid_id = parts[1].to_string();
                if let (Ok(x), Ok(y)) = (parts[2].parse::<usize>(), parts[3].parse::<usize>()) {
                    log_test_injection("button_injection", &format!("{}:{},{},{},{}", source_file, parts[0], grid_id, x, y));
                    return Some(TestButtonEvent {
                        grid_id,
                        x,
                        y,
            ...
            _ => {
                log_test_injection("button_injection_error", &format!("{}:invalid_format:{}", source_file, content));
            }
        }
...
/// Test button event for injection
#[derive(Debug, Clone)]
pub struct TestButtonEvent {
    pub grid_id: String,
    pub x: usize,
    pub y: usize,
    pub is_press: bool,
...
impl Default for TestModeInjector {
    fn default() -> Self {
        Self::new()
    }
}