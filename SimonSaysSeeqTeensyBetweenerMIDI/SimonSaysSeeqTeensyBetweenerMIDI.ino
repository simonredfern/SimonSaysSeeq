...
MIDI_CREATE_INSTANCE(HardwareSerial, Serial1, MIDI); // This was Euroshield. Not sure if this is required for USB MIDI?
...
boolean mute_gate_b = false;
  ...
  void OnNoteOn(byte channel, byte note, byte velocity){
    ...
    // MIDI Note -> GATE functionality
    GateHigh(channel); // Note: direct mapping channel to port (for now)
  ...
  void OnNoteOff(byte channel, byte note, byte velocity){
    ...
    GateLow(channel); // Note: direct mapping channel to port (for now)
...
/////////////
  ...
  //Serial.println(String("Before usbMIDI.read"));
  // Please note: DIN Midi is disabled in the betweener library. So we are using USB MIDI.
...
// Used in original use case code
void GateAHigh() {
...
void GateHigh(uint8_t port) {
  b.writeCVOut(port, 4095); //cvout selects channel 1 through 4; value is in range 0-4095
  digitalWrite(teensy_led_pin, HIGH);
}

void GateLow(uint8_t port) {
  b.writeCVOut(port, 0);
  digitalWrite(teensy_led_pin, LOW);
}
